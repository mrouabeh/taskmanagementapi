...
export const taskStatus = pgEnum("task_status", ["todo","in_progress","in_review","done","cancelled"])
...
export const tasks = pgTable("tasks", {
	...
	assigneeId: bigint("assignee_id", { mode: 'number' }).references(() => users.id, { onDelete: "set null" } ),
	createdById: bigint("created_by_id", { mode: 'number' }).references(() => users.id, { onDelete: "set null" } ),
...
export const comments = pgTable("comments", {
	...
	index("comments_task_id_idx").using("btree", table.taskId.asc().nullsLast()),]);
