...
beforeAll(async () => {
  ...
  await reset()
  const x = await outsider
    .post('/auth/register')
    .send({ name: 'x', email: outsiderEmail, password })
  ...
  projectA = (await owner.post(`/orgs/${orgId}/teams/${teamId}/projects`).send({ name: 'PA' })).body
    .project.id
  projectB = (await owner.post(`/orgs/${orgId}/teams/${teamId}/projects`).send({ name: 'PB' })).body
    .project.id
})
...
describe('tasks', () => {
  ...
  it('moves updatedAt on PATCH', async () => {
    ...
    expect(new Date(r.body.task.updatedAt).getTime()).toBeGreaterThan(
      new Date(t.updatedAt).getTime(),
    )
  })

  it('unassigns with assigneeId: null', async () => {
    const t = (await owner.post(url(projectA)).send({ title: `un-${stamp}`, assigneeId: memberId }))
      .body.task
  ...
  it('404s a task id from another project on GET, PATCH and DELETE', async () => {
    ...
    expect((await owner.patch(`${url(projectA)}/${foreign.id}`).send({ title: 'z' })).status).toBe(
      404,
    )
