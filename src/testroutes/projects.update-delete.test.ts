...
describe('PATCH', () => {
    it('renames and moves updatedAt', async () => {
        ...
        expect(new Date(r.body.project.updatedAt).getTime())
            .toBeGreaterThan(new Date(p.updatedAt).getTime())
    })
    ...
    it('409s renaming onto a sibling name, 400s an empty body', async () => {
        ...
        expect((await owner.patch(`${url(teamA)}/${p.id}`).send({ name: `taken-${stamp}` })).status).toBe(409)
...
describe('DELETE', () => {
    ...
    it('403s a plain member, who may still PATCH', async () => {
        ...
        expect((await member.patch(`${url(teamA)}/${p.id}`).send({ status: 'on_hold' })).status).toBe(200)
    })
