...
beforeAll(async () => {
    ...
    projectId = (await owner.post(`/orgs/${orgId}/teams/${teamId}/projects`).send({ name: 'P' })).body.project.id
...
const url = (t: number) => `/orgs/${orgId}/teams/${teamId}/projects/${projectId}/tasks/${t}/comments`
...
describe('comments', () => {
    ...
    it('lets an admin edit and delete somebody else\'s comment', async () => {
        ...
        expect((await owner.patch(`${url(taskA)}/${c.id}`).send({ body: 'moderated' })).status).toBe(200)
    ...
    it('moves updatedAt on PATCH', async () => {
        ...
        expect(new Date(r.body.comment.updatedAt).getTime())
            .toBeGreaterThan(new Date(c.updatedAt).getTime())
    })
