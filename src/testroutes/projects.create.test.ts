...
beforeAll(async () => {
    const keys = await redisClient.keys('rate:*'); if (keys.length) await redisClient.del(keys)
...
describe('POST /projects', () => {
    it('creates and honours an explicit status', async () => {
        const r = await owner.post(url(teamA)).send({ name: 'Alpha', status: 'active', description: 'hi' })
        expect(r.status).toBe(201)
        expect(r.body.project).toMatchObject({ name: 'Alpha', status: 'active', description: 'hi', teamId: teamA })
    })
    ...
    it('404s a team in another org', async () => {
        const other = (await owner.post('/orgs').send({ name: 'Z', slug: `zz-${stamp}` })).body.organization.id
        expect((await owner.post(`/orgs/${other}/teams/${teamA}/projects`).send({ name: 'C' })).status).toBe(404)
    })
