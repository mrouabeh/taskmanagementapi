...
projectRouter.post('/',requireRole('member'),async(req,res)=>{
    ...
    try {
        const created = await db.transaction(async (tx) => {
            await tx.select({ id: teams.id })
                .from(teams).where(eq(teams.id, req.team!.id)).for('update')
        ...
        if (code === '23505') throw new ConflictError('A project with this name corresponding to the team already exists')
        throw err
    }
})
...
projectRouter.patch('/:projectId',requireRole('member'),async(req,res)=>{
    ...
    res.status(200).json({ success: true, project: updated })
        ...
        const code = (err as { code?: string })?.code
            ?? (err as { cause?: { code?: string } })?.cause?.code
        if (code === '23505') throw new ConflictError('A project with this name already exists in this team')
        throw err
    }
